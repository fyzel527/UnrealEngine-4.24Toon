...
void UMaterialGraph::RebuildGraph()
{
	...
	if (!MaterialFunction)
	{
		...
		// Toon vars
		MaterialInputs.Add(FMaterialInputInfo(GetSpecularOffsetPinName(), MP_SpecularOffset, GetSpecularOffsetPinName()));
		MaterialInputs.Add(FMaterialInputInfo(GetSpecularRangePinName(), MP_SpecularRange, GetSpecularRangePinName()));
		MaterialInputs.Add(FMaterialInputInfo(GetShadowColorPinName(), MP_ShadowColor, GetShadowColorPinName()));
...
FText UMaterialGraph::GetMetallicPinName() const
{
	TArray<TKeyValuePair<EMaterialShadingModel, FString>> CustomPinNames({
		{MSM_Hair, "Scatter"},
		{MSM_ToonHair, "Specular Lobe2 Strength"},
		{MSM_ToonAniso, "Offset"}});
...
FText UMaterialGraph::GetSpecularOffsetPinName() const
{
	TArray<TKeyValuePair<EMaterialShadingModel, FString>> CustomPinNames(
		{ {MSM_ToonHair, "Offset"},
		{MSM_ToonAniso, "Anisotropic Roughness"} });

	return FText::FromString(GetPinNameFromShadingModelField(Material->GetShadingModels(), CustomPinNames, "Specular Offset"));
}

FText UMaterialGraph::GetShadowColorPinName() const
{
	TArray<TKeyValuePair<EMaterialShadingModel, FString>> CustomPinNames({
		{MSM_ToonHair, "Aniso Tangent"},
		{MSM_ToonAniso, "Aniso Tangent"},
		{MSM_Anisotropic, "Aniso Tangent"},
		{MSM_ToonSkin, "Subdermal Color"}});

	return FText::FromString(GetPinNameFromShadingModelField(Material->GetShadingModels(), CustomPinNames, "Shadow Color"));
}

FText UMaterialGraph::GetSpecularRangePinName() const
{
	return Material->IsUIMaterial() ? LOCTEXT("UISpecular", "UI Specular") : LOCTEXT("SpecularRange", "Specular Range");
}
...
{ 
	if( Index == 0 )
	{
		TArray<TKeyValuePair<EMaterialShadingModel, FString>> CustomPinNames({
			...
			{MSM_Cloth, "Cloth"},
			{MSM_Eye, "Iris Mask"},
			{MSM_Toon, "Shadow Lightener"},
			{MSM_ToonSkin, "SSS Mode"},
			{MSM_ToonHair, "Tighten Specular"},
			{MSM_ToonAniso, "Anisotropy"},
			{MSM_Anisotropic, "Anisotropy"}});
	...
	else if( Index == 1 )
	{
		TArray<TKeyValuePair<EMaterialShadingModel, FString>> CustomPinNames({
			...
			{MSM_Toon, "Offset"},
			{MSM_ToonSkin, "Offset"},
			{MSM_ToonHair, "Scatter"},
			{MSM_ToonAniso, "Anisotropic Rotation"},
			{MSM_Anisotropic, "Anisotropic Rotation"}});
